...
class KatProvider : MainAPI() {
    ...
    private suspend fun emitEpisodeLinks(
        ...
        for ((index, link) in links.withIndex()) {
            ...
            try {
                ...
                // Try custom extraction first for known platforms
                val customExtracted = when {
                    link.contains("streamtape.com") -> tryExtractStreamtapeUrl(link, episodeData.playUrl, countingCallback)
                    link.contains("hglink.to") || link.contains("hanerix.com") -> tryExtractHgLinkUrl(link, episodeData.playUrl, countingCallback)
                    else -> false
                }
                
                if (!customExtracted) {
                    Log.d("KatProvider", "emitEpisodeLinks: Custom extraction failed, trying loadExtractor for $index")
    ...
    private suspend fun tryExtractStreamtapeUrl(
        url: String,
        referer: String,
        callback: (ExtractorLink) -> Unit,
    ): Boolean {
        return try {
            Log.d("KatProvider", "tryExtractStreamtapeUrl: Fetching $url")
            val text = getText(url, referer)
            
            // Pattern 1: Look for direct MP4/m3u8 URLs in script tags
            var match = Regex("""(?:"|\')(?:file|src|url)(?:"|\')?\s*:\s*(?:"|\')([^"']+\.(?:mp4|m3u8)[^"']*)""", RegexOption.IGNORE_CASE).find(text)
            
            // Pattern 2: Look for HLS/streaming URLs embedded anywhere
            if (match == null) match = Regex("""https?://[^"'\s>]+\.(?:m3u8|mp4)[\w\-._?=&]*""").find(text)
            
            // Pattern 3: Look for robotjs initialization with video URL
            if (match == null) match = Regex("""startAfterSplash[^}]*?(?:"|\')([^"']+\.(?:m3u8|mp4)[^"']*)""", RegexOption.IGNORE_CASE).find(text)
            
            if (match != null) {
                val videoUrl = match.groupValues.getOrNull(1) ?: match.value
                val cleanUrl = videoUrl.trim().takeIf { it.isNotEmpty() && it.contains("http") } ?: return false
                
                Log.d("KatProvider", "tryExtractStreamtapeUrl: Found video URL: ${cleanUrl.take(60)}")
                callback(
                    ExtractorLink(
                        source = "StreamTape",
                        name = "StreamTape (Custom)",
                        url = cleanUrl,
                        referer = url,
                        quality = 80,
            ...
            Log.d("KatProvider", "tryExtractStreamtapeUrl: No video URL found in page source")
            false
        } catch (e: Exception) {
            Log.d("KatProvider", "tryExtractStreamtapeUrl: Exception - ${e.message}")
            false
        }
    }

    private suspend fun tryExtractHgLinkUrl(
        url: String,
        referer: String,
        callback: (ExtractorLink) -> Unit,
    ): Boolean {
        return try {
            Log.d("KatProvider", "tryExtractHgLinkUrl: Fetching $url")
            val text = getText(url, referer)
            
            // Pattern 1: Look for direct MP4/m3u8 URLs in JSON or script context
            var match = Regex("""(?:"|\')(?:file|src|url|sources|source)(?:"|\')?\s*:\s*(?:"|\')([^"']+\.(?:mp4|m3u8)[^"']*)""", RegexOption.IGNORE_CASE).find(text)
            
            // Pattern 2: Look for standard HLS/streaming URLs
            if (match == null) match = Regex("""https?://[^"'\s>]+\.(?:m3u8|mp4)[\w\-._?=&]*""").find(text)
            
            // Pattern 3: Look for "file" key-value pairs that might contain video URL
            if (match == null) match = Regex("""["\']?file["\']?\s*:\s*["\']([^"']+)""", RegexOption.IGNORE_CASE).find(text)
            
            // Pattern 4: Look for Plyr configuration
            if (match == null) match = Regex("""new\s+Plyr[^}]*?["\']?sources?["\']?\s*:\s*\[\s*\{[^}]*["\']src["\']?\s*:\s*["\']([^"']+)""", RegexOption.IGNORE_CASE).find(text)
            
            if (match != null) {
                val videoUrl = match.groupValues.getOrNull(1) ?: match.value
                val cleanUrl = videoUrl.trim().takeIf { it.isNotEmpty() && (it.contains("http") || it.startsWith("/")) } ?: return false
                
                val fullUrl = if (!cleanUrl.startsWith("http")) {
                    // Relative URL - make it absolute
                    "https://${url.substringAfter("//").substringBefore("/")}$cleanUrl"
                } else {
                    cleanUrl
                }
                
                Log.d("KatProvider", "tryExtractHgLinkUrl: Found video URL: ${fullUrl.take(60)}")
                callback(
                    ExtractorLink(
                        source = "HGLink",
                        name = "HGLink (Custom)",
            ...
            Log.d("KatProvider", "tryExtractHgLinkUrl: No video URL found in page source")
            false
        } catch (e: Exception) {
            Log.d("KatProvider", "tryExtractHgLinkUrl: Exception - ${e.message}")
            false
        }
    }
